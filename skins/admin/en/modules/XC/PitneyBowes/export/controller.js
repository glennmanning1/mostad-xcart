    ...
    jQuery('.export-progress .bar')
      ...
      .bind(
        ...
        function() {
          ...
          self.location = URLHandler.buildURL({ 'target': 'catalog_extraction', 'action': 'export_finished', 'status': 0 });
        }
      )
      .bind(
        'complete',
        function() {
          if (!this.errorState) {
            self.location = URLHandler.buildURL({ 'target': 'catalog_extraction', 'action': 'export_finished', 'status': 1 });
          }
        }
      );
