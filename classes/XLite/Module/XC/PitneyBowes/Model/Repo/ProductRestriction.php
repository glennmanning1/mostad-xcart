...
/**
 * ProductRestriction repository
 *
 */
class ProductRestriction extends \XLite\Model\Repo\ARepo
{
    /**
    ...
    {
        if ($value) {
            if (is_object($value) && $value instanceof \XLite\Model\Product) {
                $value = $value->getProductId();
            }
            $queryBuilder->linkInner('p.product', 'pr');

            $queryBuilder->andWhere('pr.product_id = :productId')
