...
abstract class Products extends \XLite\Logic\Import\Processor\Products implements \XLite\Base\IDecorator
{
    ...
    protected function getCurrentSKU()
    {
        return isset($this->currentRowData['sku'])
            ? $this->currentRowData['sku']
            : '';
    }
     ...
     * @return boolean 
     */
    protected function hasManualPinCodes()
    {
        ...
        return $product && $product->hasManualPinCodes();
    }
    ...
    protected function verifyData(array $data)
    {
        if ($this->hasManualPinCodes()) {
            $this->addWarning('PRODUCT-MANUAL-PIN-FMT', array('column' => $column, 'value' => $this->getCurrentSKU()));
        }
