...
class Order extends \XLite\Model\Order implements \XLite\Base\IDecorator
{
    ...
    public function checkInventory()
    {
        ...
        $oldStatus = $this->oldPaymentStatus && $this->oldPaymentStatus->getCode()
            ? $this->oldPaymentStatus->getCode()
            : '';
        ...
        if ($newStatus) {
            $delta = null;
            if (
                in_array($newStatus, $paidStatuses)
                && (
                    !$oldStatus
                    || !in_array($oldStatus, $paidStatuses)
                )
            ) {
                $delta = 1;
