...
namespace XLite\Module\CDev\Bestsellers\Logic\Sales;
...
class Generator extends \XLite\Base implements \SeekableIterator, \Countable
{
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar(static::getSalesCancelFlagVarName(), false);
    ...
    {
        \XLite\Core\Database::getRepo('XLite\Model\TmpVar')->setVar(static::getSalesCancelFlagVarName(), true);
    ...
    public function getTickDuration()
    {
        ...
        if ($this->getOptions()->time && 1 < $this->getOptions()->position) {
            ...
            $tick = \XLite\Core\Database::getRepo('XLite\Model\TmpVar')
                ->getVar(static::getSalesTickDurationVarName());
            if ($tick) {
                $result = $tick;
            }
        }
    ...
    {
        return array(
            'XLite\Module\CDev\Bestsellers\Logic\Sales\Step\Products',
        );
    }
    ...
    protected function defineStep()
    {
        ...
        if (!\XLite\Core\Database::getRepo('XLite\Model\TmpVar')->getVar(static::getSalesCancelFlagVarName())) {
