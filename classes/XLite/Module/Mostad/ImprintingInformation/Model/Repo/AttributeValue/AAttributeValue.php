...
namespace XLite\Module\Mostad\ImprintingInformation\Model\Repo\AttributeValue;
...
abstract class AAttributeValue extends \XLite\Model\Repo\AttributeValue\AAttributeValue implements \XLite\Base\IDecorator
{
    ...
    public function findAllImprintingAttributeValues(\XLite\Model\Product $product, $attributes)
    {
        $result = array();

        if ($attributes) {
            foreach ($attributes as $attribute) {
                $value = $this->findOneByValue($product, $attribute, 1);
                if ($value) {
                    $result[] = $value;
                }
            }
