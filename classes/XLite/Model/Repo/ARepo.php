...
abstract class ARepo extends \Doctrine\ORM\EntityRepository
{
     ...
     * @return boolean
     */
    public function isIndexExists($tableName, $indexName)
    {
    ...
    protected function getNotExistedIndexes(array $schema)
    {
        ...
        $thisClosure = $this; // PHP 5.3 workaround

        return array_reduce(
            $schema,
            function ($carry, $schemaLine) use ($pattern, &$thisClosure) {
                if (preg_match($pattern, $schemaLine, $matches)) {
                    $indexName = $matches[1];
                    $tableName = $matches[2];

                    $indexExists = $thisClosure->isIndexExists($tableName, 'IDX_' . $indexName);
